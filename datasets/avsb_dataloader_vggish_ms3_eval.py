...
class S4Dataset(Dataset):
    ...
    def __init__(self, split='val', args=None):
        super(S4Dataset, self).__init__()
        self.split = split
        self.args = args
        self.mask_num = 1 if self.split == 'train' else 5
        
        df_all = pd.read_csv(self.args.config['AVSBenchMS3']['ANNO_CSV'], sep=',')
        self.df_split = df_all[df_all['split'] == split]
        self.df_split = self.df_split   #[:10]
    ...
    def __getitem__(self, index):
        ...
        video_name, _ = df_one_video[0], df_one_video[1]
        
        img_base_path =  os.path.join(self.args.config['AVSBenchMS3']['DIR_IMG'])
        mask_base_path = os.path.join(self.args.config['AVSBenchMS3']['DIR_MASK'], self.split, video_name)
        ...
        for img_id in range(1, self.mask_num + 1):
            img, img_PIL = load_image_in_PIL_to_Tensor(os.path.join(img_base_path, video_name, "%s.mp4_%d.png"%(video_name, img_id)), transform=self.img_transform)
            imgs.append(img)
